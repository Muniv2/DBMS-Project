...
class MainApp:
    def __init__(self):
        ...
        # Stacked widget for login-register-listener windows
        self.stacked_widget = QStackedWidget()
    ...
    # -------------------- LISTENER: PURCHASE FLOW --------------------
    def open_purchase_screen(self, song_data):
        """Opens PurchaseWindow with song details."""
        self.purchase_window = PurchaseWindow(self)
        self.purchase_window.load_song_details(song_data)

        if self.stacked_widget.indexOf(self.purchase_window) == -1:
            self.stacked_widget.addWidget(self.purchase_window)

        self.stacked_widget.setCurrentWidget(self.purchase_window)

    def open_checkout_screen(self, final_order_data):
        """Opens CheckoutWindow with order summary."""
        self.checkout_window = CheckoutWindow(self)
        self.checkout_window.load_checkout_details(final_order_data)

        if self.stacked_widget.indexOf(self.checkout_window) == -1:
            self.stacked_widget.addWidget(self.checkout_window)

        self.stacked_widget.setCurrentWidget(self.checkout_window)

    # -------------------- LOGOUT --------------------
    def logout_user(self):
        # Close uploader windows
        if self.uploader_dashboard and self.uploader_dashboard.isVisible():
            self.uploader_dashboard.close()
            self.uploader_dashboard = None
